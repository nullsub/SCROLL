...
class CachedJastAddRoleGraph(checkForCycles: Boolean = true) extends JastAddRoleGraph(checkForCycles) with Memoiser {
  ...
  override def detach(other: RoleGraph): Unit = {
    require(other.isInstanceOf[CachedJastAddRoleGraph], MERGE_MESSAGE)
    super.detach(other)
    resetAll()
  }
